...
app.use(cors({origin: ['http://localhost:3000', 'http://localhost:5173', "https://zanith-server.onrender.com", "https://zanith.me"], credentials: true}));
...
app.post("/signup", async (req, res) => {
    ...
    if(searchUsernameArray.length === 0){
        ...
        await users.create({username: req.body.username, password: hashedPassword, email: req.body.email, displayName: req.body.displayName, admin: false, moderator: false, recordLabel: false, sessionId: sessionId, dateCreated: new Date()});
...
function getSessionIdFromCookies(cookie)
{
